      ...
      {posts.map((post) => {
        ...
        return (
          <a
            ...
            className="relative group block h-12 bg-sable overflow-hidden rounded-lg"
          >
            {imageUrl && (
              <Image
                src={imageUrl}
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end p-4">
              <p className="text-white text-sm line-clamp-3 font-medium">{post.caption || "Voir sur Instagram →"}</p>
            </div>
          </a>
        );
      })}
