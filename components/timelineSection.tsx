...
export function TimelineSection({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative max-w-5xl mx-auto py-12 px-4 overflow-x-hidden">
      <div>{children}</div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div
        ...
        group-first:top-14 
        group-last:bottom-[calc(100%-52px)]"
      />
      ...
      <div className="w-full md:w-1/2 z-10">
        <motion.div
          ...
          className={`space-y-4 pl-12 md:pl-0 ${isLeft ? "md:pr-16" : "md:pl-16"}`}
        >
      ...
      <div className="absolute left-4 md:left-1/2 top-14 md:top-14 w-6 md:w-10 h-6 md:h-10 rounded-full bg-mer -translate-x-1/2 z-20" />
